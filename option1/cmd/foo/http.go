...
import (
	...
	foo "experiments/goa-cors-override/option1/gen/foo"
	foosvr "experiments/goa-cors-override/option1/gen/http/foo/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, fooEndpoints *foo.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	// responses.
	var (
		fooServer *foosvr.Server
	)
	{
		eh := errorHandler(logger)
		fooServer = foosvr.New(fooEndpoints, mux, dec, enc, eh, nil)
		if debug {
	...
	// can happen.
	foosvr.MountFoo1Handler(mux, fooServer.Foo1)
	foosvr.MountFoo2Handler(mux, fooServer.Foo2)
	foosvr.MountFoo3Handler(mux, fooServer.Foo3)
	mux.Handle("OPTIONS", "/*", customCORSHandler(foosvr.HandleFooOrigin(fooServer.CORS), fooServer.FooOptions))
...
func customCORSHandler(ch http.Handler, fh http.Handler) http.HandlerFunc {
